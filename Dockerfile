...
LABEL org.opencontainers.image.source=https://github.com/nuntax/arbitrum-reth
...
# Extra Cargo flags
ARG RUSTFLAGS="-C link-arg=-Wl,--allow-multiple-definition"
ENV RUSTFLAGS="$RUSTFLAGS"

ENV CARGO_PROFILE_MAXPERF_LTO=thin
...
RUN if [ -n "$RUSTFLAGS" ]; then \
    ...
    # cargo build --profile $BUILD_PROFILE --features "$FEATURES" --locked --bin arb-reth
    cargo build --profile $BUILD_PROFILE --features "$FEATURES" --bin arb-reth

# ARG is not resolved in COPY so we have to hack around it by copying the
# binary to a temporary location
RUN cp /app/target/$BUILD_PROFILE/arb-reth /app/reth
